...
def make_plot(file_names,model_type,method,file_out):
    ...
    ax = gs[:,:3]
    for equil in ["equil","noneq"]:#zip([gs[:,:3],gs[:,3:]],["equil","noneq"]):
        for method in methods:
            color = plotter.c1 if equil=="equil" else plotter.c2
            results = compute_data(plotter,file_names,model_type,method,equil=equil,file_out=file_out)
            plt.subplot(ax).plot(results[0],results[1],"-v",label=r"$\mathcal{W}_{t_f}$",
                                    markersize=10,linewidth=plotter.lw,color=color)
            #plt.subplot(ax).plot(results[0],results[2],"x",label=r"$\mathcal{Q}_{t_f}$",markersize=10,linewidth=plotter.lw-1,color=plotter.c2,zorder=100,linestyle=linestyle)
            plt.subplot(gs[:,:3]).plot(results[0],results[3],"-o",label=r"$\mathcal{E}_{t_f}$",markersize=10,linewidth=plotter.lw,color=color,zorder=200)
    ...
    if model_type=="hard":
        titles = ["Stiffness as a state","Stiffness as a control"]
    else:
...
if __name__=="__main__":
    ...
    methods = ["direct"]#,"nondegenerate"]
    make_plot(file_names,model_type,methods,f"plots/test_new_cost_{model_type}_{methods[0]}.png")
