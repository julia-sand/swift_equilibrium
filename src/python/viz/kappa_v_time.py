...
def plot_kappa_vec(plotter,fig,gs,file_names,model_type,method):
    ...
    plt.subplot(gs[:,:3]).plot(results[0],results[1],"-x",label=f"g={label_val}",markersize=10,linewidth=2,color=color,zorder=100)
    plt.subplot(gs[:,3:]).plot(results2[0],results2[1],"-x",label=f"g={label_val}",markersize=10,linewidth=2,color=color,zorder=100)
