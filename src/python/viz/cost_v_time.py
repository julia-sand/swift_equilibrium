...
def plot_work(ax,tvec,workvec,plotter):
    ax.plot(tvec,workvec,"-v",label=r"$\mathcal{W}_{t_f}$",markersize=10,color=plotter.c1)
    
def plot_heat(ax,tvec,heatvec,plotter):
    ax.plot(tvec,heatvec,"-x",label=r"$\mathcal{Q}_{t_f}$",markersize=10,linewidth=2,color=plotter.c2,zorder=100)
    
def plot_ep(ax,tvec,epvec,plotter):
    ax.plot(tvec,epvec,"-o",label=r"$\mathcal{S}_{t_f}$",markersize=10,color=plotter.c3,zorder=200)
...
def plot_inset(ax_equil,results1,results2,plotter,adjust_inset):
    ...
    ax_inset.plot(results2[0], 0*results2[3],linestyle="dashed",color="gray",zorder=0)
    ax_inset.set_ylabel(r"$\Delta \mathcal{S}_{t_f}$",
                    color="maroon")
    plotter.format_ax_plain(ax_inset)
    ax_inset.plot(results2[0], results2[3]-results1[3],marker="o",markersize=10,
                label=r"$\mathcal{S}^{\boldsymbol{(b)}}_{t_f}-\mathcal{S}^{\boldsymbol{(a)}}_{t_f}$",lw=1,color="maroon")
...
def make_plot(file_names,
    ...
    if constrained_kappa!="contract":
        ...
        ax1.plot(results2[0],np.zeros(len(results2[0])),"--",color="gray",zorder=0,alpha=0.5)
        ...
        ax_noneq.plot(results2[0],np.zeros(len(results2[0])),"--",color="gray",zorder=0)
    ...
    ax_equil.plot(results1[0],np.zeros(len(results1[0])),"--",color="gray",zorder=0,alpha=0.5)
